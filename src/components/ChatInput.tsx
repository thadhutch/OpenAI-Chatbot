...
const ChatInput: FC<ChatInputProps> = ({ className, ...props }) => {
  ...
  const { mutate: sendMessage, isLoading } = useMutation({
    ...
    onError(_, message) {
      ...
      textAreaRef.current?.focus() // This will refocus the user onto the text area automatically
    }
  });
    ...
    <div {...props} className={cn("border-t border-gray-700", className)}>
      <div className="relative mt-4 flex-1 overflow-hidden rounded-lg border-none outline-none">
        <TextareaAutosize
          ...
          className="peer disabled:opacity-50 pr-14 p-1 pl-2 resize-none block w-full border-0 bg-gray-600 text-zinc-100 focus:ring-0 focus:outline text-sm sm:leading-6"
        />
